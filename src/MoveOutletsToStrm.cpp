...
//	MPI_Abort(MCW,5);
	...
	if(rank==0){
		if( sh.open(outletshapefile) ) {
			...
			nfields=sh.numberFields();
			for(i=0; i<nfields; i++) {
				field f = sh.getField(i);
				shpmoved.insertField(f,i);
			}
			//  Insert additional field to record distance moved
			field f("Dist_moved",FTInteger,6,0);
			shpmoved.insertField(f,nfields);
...
//	ismoved = new long[nxy];  // DGT decided not needed 
	...
	if(rank==0){
		for(i=0; i<nxy; i++) {
			shp = sh.getShape(i);
			xnode[i] = shp->getPoint(0).getX();
			ynode[i] = shp->getPoint(0).getY();
			origxnode[i]=xnode[i];
			origynode[i]=ynode[i];
			//  Initializing
//			ismoved[i] = 0;
...
//	delete [] tempismoved;
	...
	if(rank==0){
		for(i=0;i<nxy;++i){
			...
			shpmoved.insertShape(shpnew,i);
			shp = sh.getShape(i);
			for(j=0;j<nfields;++j){
				cell v;
				v=shp->getCell(j);  // get cell from old shape file
				shpnew -> createCell(v,j);
			}
			//  Create cell with new column
			cell v;
			v.setValue((int)dist_moved[i]);  
			shpnew ->createCell(v,nfields);
		}
